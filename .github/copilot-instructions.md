...
- [ ] 公式のパッチバージョンの変化でテストが失敗しているので修正が必要。
- [ ] Converse APIはテキスト生成時にファイルの添付に対応している。`->prompt('', attachments: [])`からUserMessageのattachmentsに渡されている。今のコードは公式のAnthropicの実装を参考にしているので`mapConverseUserMessage()`を公式の`mapUserMessage()`のようにすればいいはず。TranscriptionsのAudioBlockと似たパターン。`image` `document` `video`に対応している。 https://docs.aws.amazon.com/bedrock/latest/userguide/conversation-inference-call.html 。`InvokeModel`でも対応している。
```json
{
    "role": "user",
    "content": [
...
```
- [ ] READMEで「TranscriptionsはConverse APIなのでBedrock APIキーで使えない」ことにされてるけど特殊な使い方ではなく上の添付ファイルでAudioBlockを渡してるだけなので使えるのでは。再調査。
